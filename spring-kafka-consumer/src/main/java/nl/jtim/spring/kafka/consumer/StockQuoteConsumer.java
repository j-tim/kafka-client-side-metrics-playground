...
public class StockQuoteConsumer {
    ...
    public void on(nl.jtim.spring.kafka.avro.stock.quote.StockQuote stockQuote, @Header(KafkaHeaders.RECEIVED_PARTITION_ID) String partition) {
        log.info("Consumed from partition: {} value: {}", partition, stockQuote);

        ResponseEntity<Greeting> reponse = restTemplate.getForEntity("http://localhost:7999", Greeting.class);
    }
}
